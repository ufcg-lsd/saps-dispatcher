...
public class BaseResource extends ServerResource {
	...
	protected boolean authenticateUser(String userEmail, String userPass, boolean mustBeAdmin) {
		LOGGER.debug("Trying to authenticate the user [" + userEmail + "] with password [" + userPass + "]");
		...
		LOGGER.debug("Comparing user password in Catalog [" + user.getUserPassword() + "] to password [" + md5Pass
				+ "]");
